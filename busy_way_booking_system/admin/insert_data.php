...
if (isset($_POST['submit'])) {
	$buses_name = mysqli_real_escape_string($conn, $_POST['buses_name']);
	$drivers_name = mysqli_real_escape_string($conn, $_POST['drivers_name']);
	$schedule = mysqli_real_escape_string($conn, $_POST['schedule']);
	$categroy = mysqli_real_escape_string($conn, $_POST['categroy']);
	...
	$status = 1;
	$insert_record = mysqli_query($conn, "INSERT INTO add_bus (`buses_name`,`drivers_name`,`schedule`,`categroy`,`language`,`you_tube_link`,`action`,`decription`,`show`,`image`,`status`)VALUES('" . $buses_name . "','" . $drivers_name . "','" . $schedule . "','" . $categroy . "','" . $language . "','" . $tailer . "','" . $action . "','" . $decription . "','" . $show . "','" . $filename . "','" . $status . "')");
	if (!$insert_record) {
		echo "unsuccesfull";
	} else {
...
if (isset($_POST['updatemovie'])) {
	...
	$edit_buses_name = mysqli_real_escape_string($conn, $_POST['edit_buses_name']);
	$edit_directer_name = mysqli_real_escape_string($conn, $_POST['edit_directer_name']);
	$edit_categroy = mysqli_real_escape_string($conn, $_POST['edit_categroy']);
	...
	$insert_record = mysqli_query($conn, "UPDATE `add_bus` SET `buses_name` = '$edit_buses_name', `drivers_name` = '$edit_directer_name', `categroy` = '$edit_categroy', `language` = '$edit_language',`you_tube_link` = '$tailer',`action` = '$action',`decription` = '$decription', `show` = '$edit_show', `image` = '$image' WHERE `id` = '$e_id'");
...
if (isset($_POST['deletemovie'])) {
	...
	$sql = mysqli_query($conn, "DELETE FROM add_bus WHERE id=$id");
	echo "<script> window.location.href='add-bus.php' </script>";
}
...
if (isset($_POST['addshow'])) {
	...
	$status = 1;
	$insert_record = mysqli_query($conn, "INSERT INTO time_schedule (`show`,`theater`)VALUES('" . $show . "','" . $theater . "')");
	if (!$insert_record) {
		echo "hii";
	} else {
...
if (isset($_POST['updatetime'])) {
	...
	$insert_record = mysqli_query($conn, "UPDATE `time_schedule` SET `theater` = '$edit_screen', `show` = '$edit_time' WHERE `id` = '$e_id'");
...
if (isset($_POST['deletetime'])) {
	...
	$sql = mysqli_query($conn, "DELETE FROM time_schedule WHERE id=$id");
	echo "<script> window.location.href='time.php' </script>";
}
...
if (isset($_POST['customers'])) {
	...
	$status = 1;
	$insert_record = mysqli_query($conn, "INSERT INTO customers (`uid`,`bus`,`show_time`,`seat`,`totalseat`,`price`,`payment_date`,`booking_date`,`ticket_id`)VALUES('" . $username_id . "','" . $bus . "','" . $show_time . "','" . $seat . "','" . $totalseat . "','" . $price . "','" . $payment . "','" . $booking . "','" . $ticket_id . "')");
	if (!$insert_record) {
		echo "hii";
	} else {
